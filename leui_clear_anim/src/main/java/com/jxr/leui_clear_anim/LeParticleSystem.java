 ...
 * </pre>
 *
 * @author wangruiduan@le.com create by 2016/08/10
 */
public class LeParticleSystem implements Handler.Callback {

    private static final String TAG = "LeParticleSystem";
    ...
    public LeParticleSystem(ViewGroup mParentView, Drawable drawable, int maxParticles, int timeToLive, float displayScale) {
        mRandom = new Random();
        ...
        if (drawable instanceof BitmapDrawable) {
            ...
            for (int i = 0; i < mMaxParticles; i++) {
                mParticles.add(new LeParticleSystem.Particle(bitmap));
            }
        }
    ...
    public LeParticleSystem addModifier(ParticleModifier modifier) {
        mModifiers.add(modifier);
        return this;
    }

    public LeParticleSystem setSpeedByComponentsRange(float speedMinX, float speedMaxX, float speedMinY, float speedMaxY) {
        mInitializers.add(new SpeedByComponentsInitializer(dpToPx(speedMinX), dpToPx(speedMaxX), dpToPx(speedMinY), dpToPx(speedMaxY)));
        return this;
    }

    public LeParticleSystem setParentViewGroup(ViewGroup viewGroup) {
    ...
    public LeParticleSystem emit() {
        configureEmitter(mParentView);
        startEmitting(mMaxParticles, MESSAGE_LOOP_EMIT);
        return this;
    }
     ...
     * @param view view
     */
    private void configureEmitter(View view) {
        int[] location = new int[2];
        view.getLocationInWindow(location);
        mEmiterXMin = location[0] + view.getWidth() / 2 - mParentLocation[0];
        mEmiterXMax = mEmiterXMin;
        mEmiterYMin = location[1] + view.getHeight() / 2 - mParentLocation[1];
        mEmiterYMax = mEmiterYMin;
        Log.i(TAG, "configureEmitter.. mEmiterXMin: " + mEmiterXMin + ", mEmiterYMax: " + mEmiterYMax);
    }

    private void startEmitting(int particlesPerSecond, int messageType) {
    ...
    public static class Particle {
        ...
        public boolean update(long milliseconds) {
            ...
            if (realMilliseconds > mTimeToLive || !isRun) {
                return false;
            }
            ...
            for (int i = 0; i < mModifiers.size(); i++) {
                if (!mModifiers.get(i).apply(Particle.this, realMilliseconds)) {
                    this.isRun = false;
                }
            }
            return this.isRun;
    ...
    public interface ParticleModifier {
         ...
         * @param miliseconds
         */
        boolean apply(LeParticleSystem.Particle particle, long miliseconds);
    ...
    public static class ScaleModifier implements ParticleModifier {
        ...
        @Override
        public boolean apply(LeParticleSystem.Particle particle, long miliseconds) {
